...
+ 接手收钱吧福利社app、微信端h5和小程序项目
+ 将之前工作中的比较好的技术实践引入到新项目中（自动化打包部署、eslint、debug库等）
+ 日常业务需求开发，客服工单处理

## 同程艺龙（ 2017年3月 ~ 2020年10月 ）

职位：交通事业群 - 慧行研发前端owner
...
项目介绍：
1.使用原生小程序开发，提供车次、车站点评功能，
2.提供点到点交通出行查询，火车票、飞机票、汽车票预订功能；

- 二次封装get/post方法，封装用户信息查询，提供请求队列功能，并配置优先级(1.4版本已支持排队)
- 提供一个全局eventbus供组件之间通信
- 使用editor发布点评rich-text显示点评内容
- 使用camera组件自定义摄像头，拍照上传图片
...
- 原项目使用蜗牛内部框架sky、jquery，新需求引入vuejs渐进式开发
- 使用jsbridge和客户端进行交流
- 使用nginx作为本地开发服务器,使用nginx反向代理实现接口的访问
- 编写gulp打包脚本，使用gulp-watch结合gulp的less/cssmin/uglify等插件进行文件实时监控，修改完成后自动重新打包
